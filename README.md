
#PingMe — Real-Time Chat App

A full-stack real-time chat application built with **MERN (MongoDB, Express, React, Node.js)** and **Socket.io** for instant messaging.  
Watch demo 👉 https://youtu.be/Xb15C43Y4hU

---

## 🚀 Features

- 🔐 User Registration & Login  
- 💬 Real-time 1-on-1 messaging  
- 📸 Image upload support (Cloudinary)  
- 🔥 Online / Offline user status  
- 📍 Modern UI with Tailwind CSS  
- 🔑 JWT authentication + password hashing

---

## 🧠 Tech Stack

**Backend**
- Node.js  
- Express.js  
- MongoDB + Mongoose  
- Socket.io (real-time messaging)  
- JWT & bcryptjs  
- Cloudinary (images)

**Frontend**
- React.js  
- Vite  
- Tailwind CSS  
- Axios  
- socket.io-client

## 📸 Screenshots

<p align="center">
  <img src="https://github.com/user-attachments/assets/dcad1d79-fa2b-4a6a-8571-447448d7b1aa" width="48%" />
  <img src="https://github.com/user-attachments/assets/d3a59115-ffd8-4dec-a589-8c7ec389727f" width="48%" />
</p>

<p align="center">
  <img src="https://github.com/user-attachments/assets/7517932e-40ef-4a46-9806-bfead2f718f5" width="48%" />
  <img src="https://github.com/user-attachments/assets/c1d422c8-3db8-48cf-8e51-2077e3fca580" width="48%" />
</p>

<p align="center">
  <img src="https://github.com/user-attachments/assets/39057292-973b-4118-ba0a-d3e01e4f46dd" width="48%" />
  <img src="https://github.com/user-attachments/assets/e471138d-b0ef-4656-985b-c10aadc734e3" width="48%" />
</p>
---

## 📦 Installation

### 1. Clone repo  
git clone https://github.com/yourusername/pingme.git

2. Install dependencies

cd server
npm install

cd ../client
npm install

3. Environment Variables

Create .env for both server and client with:

MONGO_URI=your_mongo_db_connection
JWT_SECRET=your_jwt_secret
CLOUDINARY_CLOUD_NAME=
CLOUDINARY_API_KEY=
CLOUDINARY_API_SECRET=

4. Run locally

# Server
npm run dev

# Client
npm run dev


⸻

📚 Project Structure

📦pingme
 ┣ 📂server
 ┃ ┣ server.js
 ┃ ┣ routes/
 ┃ ┣ controllers/
 ┃ ┗ models/
 ┗ 📂client
   ┣ src/
   ┣ App.jsx
   ┗ index.css


⸻

⭐ Contribute

Contributions are welcome!
	1.	Fork the repo
	2.	Create a new branch
	3.	Open a PR

⸻

📝 License

MIT © Saikat Das
